---
title: "fft.Rmd"
author: "Alicia Key"
date: "2022-12-24"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Libraries

```{r}
library(magrittr)
library(ggplot2)
```

## Set a theme

```{r}
theme_set(theme_linedraw(base_size = 15))
```

## Functions for generating waveforms

```{r}
generate_waveform <- function(freq_hzs, amplitudes, decay_tau = NA, duration_s = 1.0, sps = 200) {
  t_samples <- seq(sps * duration_s)
  waveform_matrix = matrix(nrow = length(freq_hzs), ncol = length(t_samples))
  
  for (i in seq_along(freq_hzs)) {
    if (is.na(decay_tau)) {
      waveform_matrix[i, ] <- amplitudes[i] * sin(2 * pi * freq_hzs[i] * t_samples / sps)
    }
    else {
      waveform_matrix[i, ] <- amplitudes[i] * sin(2 * pi * freq_hzs[i] * t_samples / sps) * exp(-t_samples / (decay_tau * sps))
    }
  }
  
  wv <- colSums(waveform_matrix)
  
  data.frame(
    t_samples = t_samples,
    waveform = wv
  )
}
```

## Generate a more complex waveform

```{r}
# waveform_2 <- generate_waveform(c(4.0), c(1.0), decay_tau = 0.25)
waveform_2 <- generate_waveform(c(4.0), c(1.0))
ggplot(waveform_2, aes(x = t_samples, y = waveform)) +
  geom_line()
```

```{r}
y <- fft(waveform_2$waveform)
y
```
